...
class ExcelAgentSupervisor:
    ...
    def _planner_node(self, state: AgentState) -> AgentState:
        ...
        - depends_on: list of operation_ids this depends on (null if none, ["op1"] for single dependency, ["op1", "op2"] for multiple)
        ...
        [
            ...
            {{
                "operation_id": "op3",
                "tool_name": "groupby",
                "description": "Group sales by product",
                "parameters": {{
                    "group_by": "product",
        ...
        - Use depends_on as a list: null, ["op1"], or ["op1", "op2"]
    ...
    def _executor_node(self, state: AgentState) -> AgentState:
        ...
        if current_op.depends_on:
            missing_deps = [dep for dep in current_op.depends_on if dep not in state["completed_operations"]]
            if missing_deps:
                state["error_message"] = f"Dependencies {missing_deps} not completed"
                return state
        ...
        if current_op.depends_on:
            ...
            last_dep = current_op.depends_on[-1]
            prev_result = state["operation_results"][last_dep]
